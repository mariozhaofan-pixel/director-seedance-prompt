...
| 文件 | 用途 | 检索触发 |
| --- | --- | --- |
| 01_CORE_EXECUTION_CONTRACT.md | 最高行为契约、Seedance 硬约束、静默流程、续写证据门、节拍防火墙 | 所有复杂任务、续写、下一段、长项目 |
| 02_REFERENCE_ROUTER_AND_LOCKS.md | @图片 / @视频 / @音频 用途、图文冲突、声线锁定、正史参考与临时状态 | 用户上传素材、提到参考、配音、卡点、上一段视频/尾帧 |
...
| 06_TIMELINE_EDITING_SOUND.md | 时间轴、切镜、转场、声音叙事、音频卡点、节拍防火墙、实际结尾优先 | 多段分镜、硬切、音频、节奏、音乐、续写 |
| 07_STYLE_GENRE_FAMOUS_SCENE_COMPILER.md | 名场面感觉、类型语法、视觉反转 | 用户说“像某电影/动漫/名场面/感觉” |
| 08_MODEL_QC_REPAIR.md | 模型适配、Seedance QC、模型预算、生成结果复盘、故障修复 | 输出前检查、用户要求修复提示词、生成结果不理想 |
...
- 参考素材默认只写控制维度；只有明确冲突、漂移、授权、音频泄漏或多素材混淆风险时才写“不控制什么”。
