...
| 类别 | 含义 | 输出规则 |
| --- | --- | --- |
| 已发生 | 用户已接受的视频/尾帧已经表现过 | 只留在静默台账；完整视频可确认剧情/动作状态，静态尾帧只转译角色世界站位与相互距离 |
...
续写、下一段、extend、repair tail 必须先确认已接受视频、已接受尾帧或用户给出的精确可见结尾描述。完整视频用于核对剧情与动作状态；静态末尾帧只用于核对角色世界站位与相互距离。
...
已接受完整视频作为分析证据时，静默台账可记录已完成剧情、角色状态、动作相位和声音相位。只有用户明确分配时，视频才控制本段的动作、节奏或声音维度。

静态末尾帧作为本段 `@图片` 参考时，使用位置专用路由：

- 末尾帧只控制角色世界站位与相互距离。
- 角色身份/服装由原角色参考或文字控制。
- 场景拓扑、材质、光线和陈设由原剧本文字、已验收场景设定或原始场景图控制。
- 当前动作、朝向、接触与表演由本段剧本和调度控制。
- `CUT 1` 明确写 `hard cut/direct cut` 到新的摄影机起点、看向、焦段和景别，再重算前中后景、screen-left/right、人物角度与焦点路径。
- 静态末尾帧不提供 open motion、camera phase 或 audio phase；这些信息只可来自完整视频、用户文字或本段重新设计。

稳定锁定句：

```text
@图片N 命名为尾帧位置A，只控制角色在场景世界坐标中的站位与相互距离。角色与服装由角色参考A控制；场景由[剧本文字/场景参考A]完整重建；本段首镜直接切到新机位和新景别。
```
...
- 完整已接受视频可让静默台账继承剪辑点的剧情、动作和声音状态；若只提供静态末尾帧，则仅继承角色世界站位与相互距离，镜头、景别、场景画面和声音在当前段重新建立。
