...
22. 续写、下一段、extend 或修尾是否从已接受视频、已接受尾帧或精确可见结尾描述开始，而不是从计划结尾开始。
23. 已完成动作是否被排除，当前段任务是否清楚，未来保留动作是否没有提前出现。
24. 每段是否只有一个主模型预算：角色一致性、动作幅度、场景密度三选一；最多一个次重点。
25. 如果同时要求近脸、复杂动作、人群、台词、可读文字、强风格，是否已经拆段或降复杂度。
