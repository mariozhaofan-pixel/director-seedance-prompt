...
- For continuation, next-part, extend, or repair-tail work, accepted footage, an accepted final frame, or an exact visible-end description establishes what actually happened. Do not continue from the planned ending if the accepted result is unknown or different.
...
- Distinguish a full accepted clip from a still final frame. A full clip may inform the silent ledger about completed action, character state, motion phase, and audio phase. A still final frame used as a generation reference controls only character world positions and mutual spacing; it does not become the new shot's frame.
- When a still final frame is used, route dimensions explicitly: canonical character references/text control identity and costume; scene text or the original canonical scene image controls world topology, materials, light, and set dressing; the final frame controls only character position relationships; the current CUT plan controls camera, lens, shot scale, composition, focus, and action. `CUT 1` must say `hard cut` or `direct cut` to a newly specified camera origin/look direction and shot scale, then recompute foreground/midground/background and screen-left/right from that new view.
...
- Every Seedance multimodal reference must have an explicit purpose inside `【角色/场景/素材锁定】`: image identity/costume/scene/prop/color/first-frame, position-only final-frame, video camera/action/rhythm/VFX/sound, or audio BGM/beat/ambience/SFX/voice. Give each locked asset a short reusable name such as `主角A`, `稻田场景`, `收割机道具`, `尾帧位置A`, `旁白声线A`, or `动作参考A`. In `【运镜与时间轴】`, refer to these locked names instead of repeatedly writing `@图片1/@视频1/@音频1`, unless a new reference first appears in that time beat. By default, user-facing prompts state only what the reference controls. State what a reference does not control only when ambiguity, multi-reference conflict, protected identity/brand/voice, audio leakage, or generation drift risk would hurt the result.
...
- a still final frame is allowed to copy its camera, shot scale, composition, scene pixels, lighting, pose, or screen-left/right instead of being restricted to character world positions and mutual spacing
...
- For continuation/next-part work, accepted footage or an accepted final frame is used to establish actual story state; a still final frame is position-only, and the new prompt directly cuts to a newly specified camera/shot scale while rebuilding the scene from text or a canonical scene reference.
