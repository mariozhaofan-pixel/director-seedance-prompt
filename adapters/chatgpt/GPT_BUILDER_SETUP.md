...
- 武戏先静默选择空手格斗、武器格斗或能量招式；混合战斗只保留一个主路由和最多一个次路由。攻击、防御、接触、反作用、恢复姿态、兵器连续性、招式蓄势和能量碰撞都必须按秒写进 `【运镜与时间轴】`，不要默认另设 `【打斗提示词】`、`【招式说明】` 或 `【动作调度】`。
...
- 用户给出失败结果时，先判断保留、后期修、单层编辑、重跑或重写；每次尽量只改一个变量。
